...
${chalk.gray('Ex: FooBar, MyCard, Accordion')}`);
    ...
    if (config.scaffold.subdirectories.length > 1) {
        ...
        const selectedSubdirectory = config.scaffold.subdirectories.find((s) => s.name === subdirectoryName);
        if (!selectedSubdirectory) {
            console.log(`The subdirectory: ${chalk.yellow(subdirectoryName)} was not found in the supplied config. Do you need to add it's name property to the ${chalk.cyan('dullaghan.config.mjs')} file?`);
    ...
    if (config.projectType === 'JSS') {
        const { scaffoldOptSelections, } = await inquirer.prompt([
            {

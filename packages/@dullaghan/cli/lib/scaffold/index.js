...
export const scaffold = async (name, options) => {
    ...
    if (!subdirectory) {
        console.log(`The subdirectory: ${chalk.yellow(subdirectoryName)} was not found in the config. Do you need to add it to the ${chalk.cyan('dullaghan.config')} file?`);
    ...
    Promise.all(Object.entries(filesToCreate).map(([key, val]) => {

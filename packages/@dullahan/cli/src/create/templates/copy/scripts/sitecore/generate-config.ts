...
/**
 * Generate config
 * The object returned from this function will be made available by importing temp/config.js.
...
export function generateConfig(configOverrides?: { [key: string]: string }): void {
  ...
  const transformedPackageJson = transformPackageConfig();
  ...
  const config = Object.assign(defaultConfig, scjssConfig, transformedPackageJson, configOverrides);
...
function transformPackageConfig() {
  if (!packageJson.config) return {};

  return {
    jssAppName: packageJson.config.appName,
    graphQLEndpointPath: packageJson.config.graphQLEndpointPath,
  };
}
